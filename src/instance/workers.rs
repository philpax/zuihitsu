...
impl BackgroundPasses {
    /// Construct with all cursors seeded to `head`, matching `boot`'s re-seed behavior: everything
    /// written so far is treated as already processed, so a restart does not re-run old passes.
    pub(crate) fn new(head: Seq) -> Self {
        Self {
            describer_cursor: Mutex::new(head),
    ...
    /// Re-seed all cursors to `head`, treating everything written so far as already processed. Called
    /// at boot after genesis writes, so the genesis-seeded `self` (which has no synthesized
    /// description yet) is not regenerated by a synchronous catch-up before any real content is written.
    pub(crate) fn reseed(&self, head: Seq) {
        *self.describer_cursor.lock() = head;
        *self.adjudicator_cursor.lock() = head;
        *self.link_inference_cursor.lock() = head;
    }

    /// The current describer cursor value, for lag reporting.
    pub(crate) fn describer_cursor_value(&self) -> Seq {
        *self.describer_cursor.lock()
    }
    ...
    pub async fn describe_catch_up(
        ...
        let (advanced, count) = run_describe_catch_up(engine, model, cursor).await?;
    ...
    pub(crate) fn baseline_describer_cursor(&self, engine: &Engine) -> Result<(), InstanceError> {
        *self.describer_cursor.lock() = engine.store.lock().head()?;
        Ok(())
    }

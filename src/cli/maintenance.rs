...
fn consolidate(client: &Client) -> Result<(), CliError> {
    ...
    tracing::info!(actions = count, "consolidation pass complete");
    Ok(())
}
...
fn canonicalize(client: &Client) -> Result<(), CliError> {
    ...
    tracing::info!(actions = count, "canonicalize pass complete");
    Ok(())
}
...
fn link_cleanup(client: &Client) -> Result<(), CliError> {
    ...
    tracing::info!(actions = count, "link cleanup pass complete");
    Ok(())
}

...
use crate::{
    ...
    memory::memory_block::{AppendOutcome, EntrySelector, LinkOptions, RelationSpec},
...
/// tests assert over.
fn attestations(events: &[EventPayload]) -> Vec<(EntryId, Teller, Visibility, Option<String>)> {
    events
        .iter()
        .filter_map(|event| match event {
...
async fn append_dedup_distinct_from_skips_the_named_entry() {
    ...
    let outcome = block
        ...
        .unwrap();
    let AppendOutcome::Appended(new_id) = outcome else {

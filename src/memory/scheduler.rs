...
//!
//! [`fire_due`] is the scheduler — it derives wake-ups from the calendar (concrete occurrences whose
...
mod tests {
    ...
    fn fires_a_future_scheduled_entry_once_due_and_is_idempotent() {
        ...
        let (mut store, mut graph) = world(
            1_000,
            vec![
                created(id, Namespace::Event.with_name("dentist")),
                appended(
                    id,
                    entry,
                    1_000,
    ...
    fn ignores_past_anchored_future_and_recurring_occurrences() {
        ...
        let (mut store, graph) = world(
            ...
            vec![
                ...
                created(past, Namespace::Event.with_name("last-week")),
                appended(
                    past,
                    EntryId::generate(),
                    5_000,
                ...
                created(future, Namespace::Event.with_name("next-week")),
                appended(
                    future,
                    EntryId::generate(),
                    5_000,
                ...
                created(recurring, Namespace::Event.with_name("standup")),
                appended(
                    recurring,
                    EntryId::generate(),
                    5_000,
    ...
    fn recurring_entry_fires_each_due_instance_and_rearms() {
        ...
        let (mut store, mut graph) = world(
            1_000,
            vec![
                created(id, Namespace::Event.with_name("standup")),
                appended(
                    id,
                    entry,
                    1_000,

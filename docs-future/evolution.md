...
The production-shaped substrate is compared against the Stage 1 reference materialiser for replay folds, audience probes, selectors, surviving payload sets, and digests. Named tests cover hidden-input non-interference, subject-guard compilation, crash recovery, five managed-live erasure surfaces, acquisition and filtering of the current authoritative ledger under the writer lock, pending blob deletion, dated descriptions that never fire, and shared-reference retention. `restore-old-backup-current-ledger` proves filtered success after a lock-held rebuild through the verified current position. `restore-missing-current-ledger` proves that unverifiable current authority leaves serving blocked with no applied position. Durable Activities and non-interference have supporting research, while runtime erasure remains implementation pending ([verification](research/2026-07-24/verification/part-b.md), [privacy research](research/2026-07-24/lanes/provenance-privacy.md), [confidence](confidence.md#adversarial-obligation-register)).
...
Return the substrate or owning contract to revision if production-shaped output diverges from the reference oracle, an identity-bearing input is missing, audience resolution is distributed among callers, managed erasure omits a live surface, restore reads the current authoritative ledger before acquiring the writer lock, restore serves without applying a verified current ledger position under the lock, unverifiable current authority does not keep serving blocked, or pending physical deletion restores authority.
...
Executable silent-member, multi-subject, hidden-aggregate, rejected-proposal-taint, shared-reference, derived-Artefact, embedding, snapshot, `restore-old-backup-current-ledger`, `restore-missing-current-ledger`, pending-live-blob-deletion, and external-copy-bounded scenarios compare production-shaped behaviour with the reference oracle. A separate structural lock-ownership test verifies that restore and ledger publication use the same writer lock and that restore holds it from current-authoritative-ledger acquisition through filtering, rebuilding, and the serving transition. This implementation-invariant test does not model or schedule concurrent ledger publication. Non-interference and retract-versus-forget are supported concepts. The complete runtime mechanism remains implementation pending ([privacy research](research/2026-07-24/lanes/provenance-privacy.md), [current leak observation](research/2026-08-06/current-system-fixes.md), [storage](../docs/events-and-storage.md)).
...
Return the privacy, erasure, or restore design to revision if a hash acts as a bearer credential, any managed-live surface is omitted, managed restore material grants authority, the current authoritative ledger cannot prevent old-backup resurrection, restore reads authority before acquiring the single-writer lock, ledger publication occurs while the lock is held, serving opens before filtering and rebuilding complete, the lock is released before serving opens, unverifiable current authority does not keep serving blocked, pending physical deletion permits a read or Activity, or external copies are reported as deleted.

### Experimental-state disposition

Discard generated payload stores, blobs, backups, projections, snapshots, and policy encodings. Retain operational fixture JSON, expected states, ledger traces, lock-ownership traces, failure traces, measurements, and decision rationales. Fail-closed evaluator disablement and terminal erasure remain final-design properties.
...
Return the design to revision if any rehearsal diverges from a structural oracle, reads restore authority before acquiring the writer lock, permits ledger publication while the lock is held, opens a serving surface before filtering and rebuilding complete, releases the lock before serving opens, opens serving without a verifiable current authoritative ledger, loses source or stable identity, violates an audience boundary, exceeds a frozen budget, or cannot recover after projection deletion or crash.
...
| Stage 2 | Can the complete substrate match the reference and enforce baseline safety? | Production-shaped substrate, audience resolver, storage partition, and restore harness | Production/reference equality; zero hidden residue; five managed-live surfaces close; `restore-old-backup-current-ledger` filters successfully under the writer lock; `restore-missing-current-ledger` remains blocked without verifiable current authority; pending-deletion denial holds. | Divergence, omitted influence, authority acquisition before the lock, serving without a verified current authoritative ledger position, or authority restoration revises the substrate or owner contract. | `genesis-replay`, `genesis-subject-guard`, `genesis-zero-residue`, `genesis-managed-erasure-closure`, `genesis-shared-reference-retention`, `erase-one-reference-retain-another`, `restore-old-backup-current-ledger`, `restore-missing-current-ledger`, `pending-live-blob-deletion`, `external-copy-bounded` | Genesis freeze |
...
| Stage 8 | Can richer privacy and operational erasure preserve the baseline contract? | Principle evaluators and operational-fixture harness | Zero prohibited flow; the current authoritative ledger filters restore; the writer lock excludes publication from authoritative-ledger acquisition through filtering, rebuilding, and the serving transition; unverifiable current authority keeps serving blocked; external copies stay bounded. | A structural lock-ownership test fails on authority acquisition before the lock, a publication path that does not require the same lock, serving before rebuild completion, or lock release before serving opens. It verifies implementation ownership and ordering without scheduling concurrent publication. Managed-restore authority, omitted live deletion, read during pending deletion, unverifiable authority that opens serving, or false external deletion also revises the contract. | `privacy-principle-matrix`, `privacy-hidden-influence`, `privacy-managed-restore`, `privacy-restore-writer-lock`, `privacy-external-accounting` | Genesis freeze for baseline mechanics; post-genesis extension per richer principle |

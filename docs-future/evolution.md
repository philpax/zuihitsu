...
| `gate:generated-episodes` | `activation_gate` | `capability:source-activity-substrate`, `capability:audience-influence-substrate` | `evidence:gate-generated-episodes` | `true` | `false` |
...
Stage 1 recasts the baseline studies as independently executable evidence packages. The packages are `evidence:1a-episode-ablation`, `evidence:1b-witness`, `evidence:1c-contract`, `evidence:1c-measurement`, `evidence:1d-budgets`, `evidence:1e-query-classification`, and `evidence:1f-multimodal-recall`. They may run in parallel after `stage:0` has fixed their inputs, metrics, thresholds, and privacy tolerances.

`1a-episode-ablation` measures generated-episode retrieval value and the non-evidence wall. `1b-witness` measures connector witness evidence, availability, presence, disclosure, and exposure assurance. `1c-contract` fixes the canonical fixture encoding and the extraction metrics before observation. `1c-measurement` measures extraction yield, convergence, latency, and economics against canonical Proposition, Assertion, and Attestation fixtures. `1d-budgets` measures browser, log, projection, blob, and eval-package cost. `1e-query-classification` classifies query surfaces on real turns. `1f-multimodal-recall` measures recall using prior Perception, explicit reinspection, and textual memory.
...
Stage 2 builds only the schema-neutral fixture compiler, correctness-first reference materialiser, canonicalisation rules, transition properties, audience probes, semantic erasure expectations, replay digests, and counterfactuals. It does not assign production storage, connector integration, or agent-facing behaviour to the stage.
...
Stage 7 defines the smallest complete experimental path. An Occasion is retained. Extraction creates a durable proposal. Hard critics evaluate the proposal. Atomic publication creates conservative actual-positive candidate structure or source-only fallback. Central audience resolution produces a rendered read. Access accounting records delivered content.
...
Stage 9 blocks Stage 10 and the initial temporal capability only when `capability:initial-temporal-policy` is selected as `initial_policy`. It does not block genesis for unselected temporal extensions.
...
Stage 10 blocks Stage 11 and the initial Event capability only when `capability:initial-event-role-relation-policy` is selected as `initial_policy`. Optional role vocabularies and autonomous merging remain isolated.
...
Stage 11 blocks Stage 12 and the initial identity capability only when `capability:initial-identity-policy` is selected as `initial_policy`. Autonomous scoring and cross-instance identity have independent gates.
...
Stage 12 produces `evidence:stage12-support-policy`. Shared-room, agent-restatement, relay and return, reliability, hidden-support, last-independent-support withdrawal, opposite-polarity, quantity, and contest-versus-contradiction fixtures compare production-shaped projections with the reference oracle. Tests include `support-hidden-zero`, `support-dependence`, `support-last-withdrawal`, and `support-contest` ([identity and belief research](research/2026-07-24/lanes/identity-belief.md), [confidence](confidence.md#evidence-map)).

### Required decisions

The belief, privacy, architecture, eval, and operator owners record ordinal vocabulary, dependence rules, reliability use, promotion threshold, contradiction registry, no-rank alternative, and the zero-tolerance privacy oracle. Decisions link to `unresolved:support-arithmetic-fusion`, `unresolved:initial-policy-matrix`, and the support obligations.
...
Stage 12 blocks the genesis freeze and the initial support capability only when `capability:initial-support-policy` is selected as `initial_policy`. Optional fusion and arbitration do not block genesis when disabled.
...
The following table is generated from the finite scan over every `docs-future/*.md` file except `docs-future/evolution.md` and every path below `docs-future/research/`. The scan uses the word-bounded marker alternatives `safely deferred`, `not enabled`, `later activation`, `post-genesis`, `open experiment`, `remains off`, `optional`, `deferred`, `disabled`, `inactive`, `gated`, and `gate`, plus the bare rules `until` and `only after`. It records one row per matching source line. The digest covers the source path, nearest source heading, and exact stripped line, so the identity does not depend on a line number.
...
| `handoff:stage1` | `stage:1` | `owner:model-eval` | `evidence:1a-episode-ablation`, `evidence:1b-witness`, `evidence:1c-contract`, `evidence:1c-measurement`, `evidence:1d-budgets`, `evidence:1e-query-classification`, `evidence:1f-multimodal-recall` | `oracle:stage1-baseline` | consuming stages and selected gates | Do the treatments have measured benefit, cost, and privacy bounds? | independent preregistered evidence packages | fixed sample, uncertainty, budget, tolerance, and pass rule | indistinguishable, over-budget, or post-hoc result | `measurement:1a-episode-ablation`, `measurement:1b-witness`, `test:1c-contract`, `measurement:1c-measurement`, `measurement:1d-budgets`, `test:1e-query-classification`, `measurement:1f-multimodal-recall` |
...
| `handoff:stage5` | `stage:5` | `owner:security` | `evidence:stage5-audience-influence`, `evidence:stage5-status-freshness` | `oracle:stage5-audience-influence` | read/write, privacy, identity, and gates | Can central audience resolution prevent hidden influence and stale action? | audience resolver and status freshness record | zero hidden residue and fail-closed stale state | distributed resolution or disclosure | `test:stage5-audience-influence`, `test:stage5-status-freshness` |
| `handoff:stage6` | `stage:6` | `owner:storage` | `evidence:stage6-erasure-recovery` | `oracle:stage6-erasure-recovery` | freeze, rehearsal, genesis | Can erasure and restore close over all managed surfaces? | storage, ledger, restore, and lock harness | zero residue, filtered restore, and blocked unverifiable authority | omitted surface, authority resurrection, or lock failure | `test:stage6-erasure-closure`, `test:stage6-restore-lock` |
| `handoff:stage7` | `stage:7` | `owner:architecture` | `evidence:stage7-vertical-slice`, `evidence:1c-measurement` | `oracle:stage7-authority` | stage 8 and initial writes | Can one path retain source, publish bounded structure, resolve audience, and account for delivery? | source-to-render vertical slice | fidelity, atomicity, source-only, audience, and budget thresholds | critic bypass, partial publication, or hidden residue | `test:stage7-vertical-slice`, `test:stage7-authority` |
| `handoff:stage8` | `stage:8` | `owner:operations` | `evidence:stage8-operational-substrate`, `evidence:stage8-disabled-worker` | `oracle:stage8-operational-substrate` | freeze and job-dependent gates | Can jobs remain bounded and disabled without losing pending state? | event-sourced runner and disabled-worker harness | one valid outcome, zero disabled execution, additive activation | duplicate, stale, unbounded, or hidden execution | `test:stage8-jobs`, `test:stage8-disabled-worker` |
...
| `handoff:stage12` | `stage:12` | `owner:belief` | `evidence:stage12-support-policy` | `oracle:stage12-support-policy` | selected initial support policy | Can support and contradiction remain conservative and audience-safe? | support and contradiction projections | zero hidden delta, no dependence gain, and mechanical classifications | hidden influence or linguistic overclaim | `test:stage12-support-policy` |
...
| `handoff:oracle-bulk-ingestion-gate` | `capability:bulk-ingestion` | `owner:ingestion` | `evidence:gate-bulk-ingestion` | `oracle:bulk-ingestion-gate` | capability activation | Does bulk ingestion preserve source, audience, and cancellation bounds? | bulk-ingestion gate harness | declared fidelity and work thresholds | source loss or unbounded work | `test:oracle-bulk-ingestion-gate` |
| `handoff:oracle-exploration-yield` | `capability:exploration` | `owner:operations` | `evidence:gate-exploration` | `oracle:exploration-yield` | capability activation | Does exploration yield useful proposals within privacy and work budgets? | exploration yield harness | preregistered yield and cost | unsolicited disclosure or low-yield work | `test:oracle-exploration-yield` |
| `handoff:oracle-habitual-modality-gate` | `capability:habitual-deontic-inference` | `owner:temporal-policy` | `evidence:gate-habitual-deontic-inference` | `oracle:habitual-modality-gate` | capability activation | Does habitual and deontic inference stay qualified? | modality gate harness | no unqualified normative output | normative invention | `test:oracle-habitual-modality-gate` |

| `handoff:oracle-historical-reinspection` | `capability:historical-reinspection` | `owner:artefacts` | `evidence:gate-historical-reinspection` | `oracle:historical-reinspection` | capability activation | Does reinspection preserve source authority and audience? | reinspection access harness | authorised source and selector only | laundering or hidden access | `test:oracle-historical-reinspection` |

| `handoff:oracle-identity-thresholds` | `capability:autonomous-identity` | `owner:identity` | `evidence:gate-autonomous-identity` | `oracle:identity-thresholds` | capability activation | Do autonomous identity thresholds preserve disjointness and disclosure safety? | identity threshold harness | threshold and clearance rules hold | false merge or disclosure effect | `test:oracle-identity-thresholds` |
...
| `handoff:oracle-proactive-initiation-safety` | `capability:proactive-initiation` | `owner:operations` | `evidence:gate-proactive-initiation` | `oracle:proactive-initiation-safety` | capability activation | Does proactive initiation remain purpose-bound and race-safe? | initiation safety harness | no action without authority and purpose | unsolicited or stale action | `test:oracle-proactive-initiation-safety` |
| `handoff:oracle-qualitative-temporal-subalgebra` | `capability:qualitative-temporal-inference` | `owner:temporal-policy` | `evidence:gate-qualitative-temporal-inference` | `oracle:qualitative-temporal-subalgebra` | capability activation | Does the qualitative temporal vocabulary remain closed and uncertain? | temporal subalgebra harness | valid composition and explicit unknown | guessed or invalid time | `test:oracle-qualitative-temporal-subalgebra` |


| `handoff:oracle-runtime-faithfulness` | `capability:general-runtime-faithfulness-checking` | `owner:model-eval` | `evidence:gate-general-runtime-faithfulness-checking` | `oracle:runtime-faithfulness` | capability activation | Does runtime checking remain observational and non-authoritative? | faithfulness checker harness | no checker authority or hidden influence | checker takeover | `test:oracle-runtime-faithfulness` |
...
| `handoff:oracle-support-fusion-gate` | `capability:support-fusion` | `owner:belief` | `evidence:gate-support-fusion` | `oracle:support-fusion-gate` | capability activation | Does support fusion suppress dependence and hidden influence? | support fusion gate harness | no dependence gain or hidden delta | privacy leak or arithmetic overclaim | `test:oracle-support-fusion-gate` |
...
| `handoff:whole-system-rehearsal` | `lifecycle:whole-system-rehearsal` | `owner:operator` | `evidence:rehearsal-report` | `oracle:whole-system-rehearsal` | first real genesis | Does a fresh instance satisfy the frozen contract under ordinary and faulted operation? | disposable instances and rehearsal report | all selected workflows and disabled checks match | any divergence, restore fault, or budget failure | `test:rehearsal-complete-path`, `test:rehearsal-delete-replay`, `test:rehearsal-restore-ledger` |
| `handoff:first-real-genesis` | `lifecycle:first-real-genesis` | `owner:operator` | `evidence:freeze-record`, `evidence:rehearsal-report` | `oracle:first-real-genesis` | permanence boundary | Do deployed inputs exactly match the approved candidate? | signed first-real-genesis-manifest | exact versions, digests, readiness, and configuration | any mismatch or stale evidence | `test:genesis-manifest-verify`, `test:genesis-readiness` |
...
| `test:oracle-bulk-ingestion-gate` | `bulk-ingestion-gate` | `handoff:oracle-bulk-ingestion-gate` |
| `test:oracle-exploration-yield` | `exploration-yield` | `handoff:oracle-exploration-yield` |
| `test:oracle-habitual-modality-gate` | `habitual-modality-gate` | `handoff:oracle-habitual-modality-gate` |
| `test:oracle-historical-reinspection` | `historical-reinspection` | `handoff:oracle-historical-reinspection` |
| `test:oracle-identity-thresholds` | `identity-thresholds` | `handoff:oracle-identity-thresholds` |
| `test:oracle-proactive-initiation-safety` | `proactive-initiation-safety` | `handoff:oracle-proactive-initiation-safety` |
| `test:oracle-qualitative-temporal-subalgebra` | `qualitative-temporal-subalgebra` | `handoff:oracle-qualitative-temporal-subalgebra` |





| `test:oracle-runtime-faithfulness` | `runtime-faithfulness` | `handoff:oracle-runtime-faithfulness` |
| `test:oracle-seed-definition-set` | `seed-definition-set` | `handoff:oracle-seed-definition-set` |
| `test:oracle-support-fusion-gate` | `support-fusion-gate` | `handoff:oracle-support-fusion-gate` |
...
| `oracle:bulk-ingestion-gate` | `owner:ingestion` | `capability:bulk-ingestion` | capability activation | `handoff:oracle-bulk-ingestion-gate` | `test:oracle-bulk-ingestion-gate` |
| `oracle:exploration-yield` | `owner:operations` | `capability:exploration` | capability activation | `handoff:oracle-exploration-yield` | `test:oracle-exploration-yield` |
| `oracle:habitual-modality-gate` | `owner:temporal-policy` | `capability:habitual-deontic-inference` | capability activation | `handoff:oracle-habitual-modality-gate` | `test:oracle-habitual-modality-gate` |
| `oracle:historical-reinspection` | `owner:artefacts` | `capability:historical-reinspection` | capability activation | `handoff:oracle-historical-reinspection` | `test:oracle-historical-reinspection` |
| `oracle:identity-thresholds` | `owner:identity` | `capability:autonomous-identity` | capability activation | `handoff:oracle-identity-thresholds` | `test:oracle-identity-thresholds` |
| `oracle:proactive-initiation-safety` | `owner:operations` | `capability:proactive-initiation` | capability activation | `handoff:oracle-proactive-initiation-safety` | `test:oracle-proactive-initiation-safety` |
| `oracle:qualitative-temporal-subalgebra` | `owner:temporal-policy` | `capability:qualitative-temporal-inference` | capability activation | `handoff:oracle-qualitative-temporal-subalgebra` | `test:oracle-qualitative-temporal-subalgebra` |
...
| `oracle:runtime-faithfulness` | `owner:model-eval` | `capability:general-runtime-faithfulness-checking` | capability activation | `handoff:oracle-runtime-faithfulness` | `test:oracle-runtime-faithfulness` |
...
| `oracle:support-fusion-gate` | `owner:belief` | `capability:support-fusion` | capability activation | `handoff:oracle-support-fusion-gate` | `test:oracle-support-fusion-gate` |

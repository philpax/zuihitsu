...
A person met on two platforms is two stubs until something says otherwise. What says otherwise is a merge. A merge is a revocable, credence-bearing assumption rather than a permanent union.

All of this lives below a wall. The agent sees one resolved handle per person. It never chooses between siblings, never tests two handles for equality, and never learns that the machinery exists.
...
A merge is a first-class object with its own credence. That credence strengthens under corroboration and weakens under divergence. A merge is not a boolean, and it is not transitive closure over an equivalence relation.
...
## Recall against disclosure

A merge licenses two different things, and they are separable.

Recall unifies history. Asking about a person returns what they said on both platforms, and the agent reasons about one person. Recall rides the tentative merge and is safe to be imperfect, because severance genuinely undoes it.

Disclosure crosses a boundary in the world: saying something to this account that was told by that one. Disclosure is not undoable, and it does not ride the same credence. It requires either evidence past a far stricter bar or a completed challenge-response, and until then the agent may know the connection without acting on it.
...
Separating these lets identity resolution be aggressive where the cost of being wrong is a re-fold, and conservative where the cost is a confidence spoken to the wrong person.
...
## Agent and operator participation

Merging is not something the agent is forbidden from participating in. The agent can observe that two people seem to be the same and say so, and that observation is evidence like any other, weighted by the fact that the agent is a fallible teller. What the agent cannot do is perform the merge, inspect the class, or route around the resolution it was handed.

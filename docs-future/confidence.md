...
| **The agent re-recording what it was told reads as a second teller** | **Open** | 77 of 198 live entries are agent-told, many of them the agent restating a participant's own sentence. Under teller-counting that is corroboration from one source. Declaring the agent a witness to everything told to it, and never an independent teller of it, is the candidate fix and is not yet stated |
| **A single teller's hedge-then-confirm arc** | **Open** | The showcase case, and [`evolution.md`](evolution.md) stage 6 gates on it, but in the live log both utterances are the same teller with no corroboration anywhere, so under this chapter's own rules the credence must not move. Either an expressed-confidence axis distinct from credence, or the arc is a supersession rather than a credence move, which reworks three chapters and the gate |
| **A speaker's hedge is not the store's credence** | **Synthesis** | Two independent tellers each saying "probably X" counts as two pieces of evidence for X under the counting rule, though neither committed to it. A speaker hedge is properly an attitude about the speaker |
...
| **The audience of an episodic narrative body** | **Open** | The five episodic rules cover teller, premises, distillation, attestation, and marking, and none covers audience. A narrative recounting a session that contained a confidence is a second read path with no visibility computed on it. Composing the body from public content only, mirroring the distillation boundary, is the consistent candidate and is not yet taken |
| **Promotion out of working memory carries no taint** | **Open** | A working note has no transmission principle by construction, so the audience-invariant critic has nothing to check when a reflection pass promotes it. A taint set naming what the note consulted is the candidate fix, and it is state the fold must reproduce, which raises the stakes on the scratchpad-storage question below |

...
| The directive kind: scope, author, lifecycle, composition | Decided | The live log's connector-minted per-context directives are the shape that forced it: neither always-in-context nor operator-owned, so neither the slot nor the fact model can hold them. The connector authority is new and explicitly bounded to per-context scope |

...
| Conceptual graphs | The confirmation that plural referents with counts are a solved representation problem | The collective-against-distributive distinction, which CGs mark natively and this design declines to pay for |
| Voyager | Procedural memory as executable code indexed by a natural-language description | The specific domain |
| MemGPT and Letta | The tiered memory framing | Model-authored state as the primary mechanism. It is a live and validated design, and this design chooses checked deterministic curation instead, at a real cost in agent autonomy over its own salience |
...
| Hindsight † | Independent corroboration of the [four-kind typology](memory-typology.md), and the parallel-lane retrieval shape whose rank fusion [the query surface](query-surface.md) adopts, a form the [2026-07-24 survey of current systems](research/2026-07-24/lanes/survey-issue7.md) had already found in a different production system | Consolidating experience upward into belief, which [the episodic wall](two-traces.md) forbids outright, and measuring success by a composite memory benchmark, for the reason in the graveyard lessons below |
| The day-dreaming loop † | The observation that a purely reactive store never notices a connection nothing prompted it to look for, and the accounting that makes ambient generation expensive: most of what it produces is waste, and the ratio is the mechanism's defining property rather than a tuning problem. Both are why [exploration](off-turn.md) is metered and structurally sampled | Random pairing, which a typed graph improves on; scoring an idea for novelty and keeping the high scorers, since one model's approval is one signal and promotion needs two; and the strategic frame of daydreaming as proprietary training data, which is a frontier-lab concern and not a personal agent's |

† These three were added after the [verification passes](research/README.md) ran, from sources read directly rather than through a research lane. They are not covered by the 2026-07-24 verification, and the rank-fusion attribution above is deliberately split: the parallel-lane shape is corroborated by the earlier survey, while the specific system in this row is a contemporary example of it rather than its source.

## Two inheritances stated in full
...
The OWL construct closest to the second is qualified cardinality, which counts fillers of a property restricted to a class. OWL 2 does allow asserting one of a named individual, so "Vertas has exactly five ice creams" is expressible there. Two things prevent borrowing the mechanism rather than the idea. OWL has no unique-name assumption, so five mentions are not five things until inequality is stated explicitly, and it has no temporal scoping, so a count that changes needs the whole assertion reified as a fluent. The count here is a value with a validity interval, which provides both without additional machinery.

The value's shape is Wikidata's: an amount, an optional unit, and optional bounds. The bounds matter more than their size suggests, because a corpus is full of approximations stated as though they were exact.

...
/// _session_carryover, same_as) covers it, so the pass must coin a new relation rather than reusing one.
...
impl Scenario for InfersLinkFromContent {
    ...
    async fn run(&self, ctx: &RunContext) -> Result<(), EvalError> {
        ...
        // The seed relations are knows, created_by/operator_of, _session_carryover/_session_carries, same_as — none

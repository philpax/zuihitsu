...
- `POST /control/merge` — the console's merge control (`confirm_merge`). The body is `{ from, to }` (the two stubs by memory id). Confirming authors the merging `same_as` (`LinkSource::Operator`) — the only path across the identity gate; there is no decline counterpart, so a proposal the operator is not convinced by simply stays pending. The console surfaces the agent's pending merge proposals here for one-click resolution, but the same control also lets the operator assert a merge it knows to be true independently of any proposal — the operator is not limited to the agent's guesses. This is the operator's *dedicated* merge endpoint; there is no other one. It is not, however, the only way operator authority reaches a `same_as`: the imprint interview's committing Lua authors one (merging `person/operator` into the operator's real profile; see [Cross-platform identity](data-model.md#cross-platform-identity-operator-asserted)). The general operator Lua console (`POST /control/lua`) runs under the same operator authority — where a `same_as` create is honoured rather than rerouted to a proposal, as a platform block's create would be — but it is a no-commit sandbox, so it persists no merge.
...
`same_as` is auto-traversed on reads: `memory.get`, search, and the link readers (`outgoing`/`incoming`/`links`) surface content and links from the whole class, deduplicated, with per-stub provenance preserved. A link reader orients every edge against the queried identity — `outgoing` for an edge the identity is the source of, `incoming` for one it is the target of — and surfaces only relationships pointing *out* of the class, never the `same_as` edges holding it together; each result carries the far memory as an actionable handle alongside the relation, direction, source, the teller who asserted it (`told_by`), and the far memory's representative occurrence (its freshest dated entry's `occurred_at`, authored outranking extracted), so a neighbourhood rendered from a hub keeps each spoke's *when* without a second read. The far endpoint (and the `told_by` teller) is itself resolved to its own class primary, so a relationship recorded against a raw platform stub renders under the neighbour's canonical readable handle rather than an opaque snowflake, and parallel edges that then coincide on the same neighbour collapse to one, so an identity linked through both a stub and its canonical profile reads once. Like the relation-registry reads, they reflect committed state, so a link written in the same block is not yet visible to a read in it. Writes are not fanned across the class, but a content write through any member's handle — a bare `person/dave` or a platform stub `dave@discord` alike — resolves to the class's primary stub, the right home for a human-fact; a stub is the default operand the agent holds, and a fact about a person belongs on the person. Naming a specific stub disambiguates a reference (`memory.get("person/dave@slack")`), not a write; a genuinely binding-scoped attribute is the connector's own, written under connector provenance.

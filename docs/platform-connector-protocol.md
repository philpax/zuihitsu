...
A platform connector bridges a platform to the agent server. It delivers participant messages, watches the agent's generation arrive, and posts the reply back. This document covers the HTTP surface: endpoints, request/response shapes, streaming format, and auth.

## Auth

Each platform connector is registered in the instance config under a top-level `[platform_connectors]` map. The map key is the platform the connector serves; every operation in a request acts on that platform, and its writes are attributed to that connector. The value carries the connector's bearer key:
...
Every `/platform/*` request is scoped to exactly one platform connector, and the key decides the scope first. A request bearing a registered connector's key (`Authorization: Bearer <key>`) resolves to the platform that connector serves, wherever it connects from, so a connector running on the same host as the server (a bot on `localhost`, the usual deployment) is still scoped to its own platform by its key, not mistaken for the operator's console. That platform is what every operation in the request acts on, and the connector is what its writes are attributed to. A request bearing no key falls back to its origin: a loopback peer is the operator's own console, scoped to the reserved `direct` platform; a remote peer is rejected with `401`. A request bearing an unrecognised key is rejected with `401` outright, so a misconfigured connector fails loudly rather than silently acting as `direct`.

Because the platform is derived from the connector's key, no request body carries one: there is nothing in the payload to spoof. The platform surface carries no operator authority: a platform connector acts only as the participants it represents, never as the operator (see [Trust and authority → Clients and the server boundary](trust-and-authority.md#clients-and-the-server-boundary)).
...
A platform connector uses this to drive a typing indicator or partial-message edits. The stream ends after the terminal frame (`outcome` or `error`). A connector that ignores every `progress` frame behaves identically to one that never upgraded.
...
- `participant` — the bare id of the joiner, resolved under the request's platform to `person/<id>@<platform>`.
...
- `scope_path` — the room's address within the connector's platform.
- `roster` — the full set of bare ids currently in the room, each resolved under the request's platform.
...
Assert — or, with `remove`, retract — a structural link between two of the platform connector's own scoped memories. The connector uses this to record placement: a channel and its members are `part_of` a guild, say. Both endpoints are resolved under the request's platform, so a connector can only ever link memories it owns.
...
The edge is `Public` (a structural fact, not a told aside) and carries `LinkSource::PlatformConnector` — the request's connector, from its key — so an audit reads which connector authored it.

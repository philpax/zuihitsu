...
One operational caveat: the event log is a WAL database, and SQLite reads a WAL database through its shared-memory index. A read-only open therefore succeeds against a cleanly closed log and against one a live writer is holding, but fails on a log left hot by a killed process, where recovering the WAL would itself be a write. Boot the instance read-write once to recover it.

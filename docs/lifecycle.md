...
The keys the config carries never cross the wire back out: `control_keys` and `platform_keys` serialise as their *count*, and an MCP server's `env` serialises as its variable *names* only, so the config view (`GET /control/config`) cannot leak a secret.

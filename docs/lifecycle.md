...
A read-only boot coexists as a separate path (`--read-only`, or `[serving] read_only = true`): the store, graph, and vector index open read-only (no writer lock, no schema guard, no DDL), `boot()` is skipped entirely (the at-rest graph is already materialized from the last live boot), no background drivers spawn, no model client is built, and every mutating handler returns `409`. The data is a snapshot at boot — the SSE stream serves its initial snapshot then idles — so the operator restarts the read-only server to see new data. The use case is frontend development against an at-rest instance: the operator starts the read-only server against the instance's storage directory (the agent is not running) and develops against the console with a guarantee that nothing will change.

...
- `[platform_connectors]` — the map of registered platform connectors, one entry per connector (`discord = { key = "…" }`). The map key is the platform the connector serves; every operation in a request acts on that platform, and its writes are attributed to that connector. The value carries the connector's bearer key. Every `/platform/*` request is scoped to exactly one platform connector, resolved from its key, and the platform it acts on is the one that connector serves, never a value in the request body. A loopback peer needs no key and is confined to the reserved `direct` platform; a remote peer with no key, or an unrecognised one, is rejected. An empty map (the default) rejects every keyed platform request, leaving only loopback's `direct` (see [Trust model](trust-and-authority.md#trust-model)).
...
The keys the config carries never cross the wire back out: `control_keys` serialises as its *count*, each platform connector's key serialises away (leaving only the platforms), and an MCP server's `env` serialises as its variable *names* only, so the config view (`GET /control/config`) cannot leak a secret.
...
LinkTypeRegistered       (member_of / has_member)            -- standing membership: a person belonging to an organisation or group (an employer, a team, a board, a school)
...
The seed relations are a minimum-viable ontology of structural universals. `located_at` (placement) earns its seat because the agent otherwise re-coins it under scattered spellings — `held_at`, `occurs_at`, `based_in` — in crash-then-register cycles; that recurrence is the bar a relation must clear to count as a universal the system leans on. `member_of` (standing membership) clears the same bar from the other direction: with no seed for belonging, the agent split it across `part_of` and `participates_in` — stretching both past their stated scope, since `part_of` explicitly disclaims people and `participates_in` means an event — and coined `works_at`, `employed_by`, and `involved_in` around the edges. A seed relation quietly absorbing meaning it disclaims is the same fragmentation as a scattered spelling, and harder to see. Remaining social semantics — mentorship and the like — are still the agent's to coin with `links.register` as its environment calls for them (see [The seed ontology](../CONTRIBUTING.md#the-seed-ontology) in `CONTRIBUTING.md`). Genesis emits no `created_by` link and no facts about anyone; a freshly-born agent genuinely doesn't know who made it.
...
A read-only boot coexists as a separate path (`--read-only`, or `[serving] read_only = true`): the store, graph, and vector index open read-only (no writer lock, no DDL, no schema *repair*), `boot()` is skipped entirely, no background drivers spawn, and no model client is built. The use case is frontend development against an at-rest instance: the operator starts the read-only server against the instance's storage directory, with the agent stopped, and develops against the console knowing nothing will change.

Mutating requests return `409`. The refusal is a per-surface middleware keyed on the request method rather than a check inside each handler, because on both surfaces every mutating route is a `POST` or a `PUT` and every read route is a `GET`: a route added later is gated by construction, with nothing to remember. The read methods are the enumerated set and everything else is refused, so an unconsidered method fails closed; `HEAD` and `OPTIONS` count as reads, since axum answers a `HEAD` from its `get` handler.

Two properties of the mode are worth stating exactly, because both differ from the "frozen snapshot" it resembles.

- **The graph and the vector index are frozen; the event log is not.** Nothing re-materialises the graph, so every graph-backed read answers from the state the last live boot left behind. The event log, by contrast, is queried afresh per request, so a concurrently running agent's newer events do appear in the log and in the SSE catch-up while the graph has not seen them. The two halves agree only while the instance is genuinely at rest, which is the intended use. The SSE stream never pushes a live tail either way — a read-only store has no subscriber feed, so the stream serves its catch-up and idles. Restart to advance the frozen half.
- **The schema stamp is checked even though it cannot be repaired.** A live boot resets a graph stamped under another build's schema and rebuilds it from the log; that repair is a write, so a read-only boot instead refuses to open, naming the two stamps and telling the operator to boot read-write once. An unstamped graph is refused on the same reasoning. Serving a projection this build did not create would be the one silent failure mode the mode could have.

One operational caveat: the event log and the graph are WAL databases, and SQLite reads a WAL database through its shared-memory index. A read-only open therefore succeeds against a cleanly closed database and against one a live writer is holding, but fails on one left hot by a killed process, where recovering the WAL would itself be a write. Boot the instance read-write once to recover it.

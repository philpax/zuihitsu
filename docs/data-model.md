...
[Storage and materialization](events-and-storage.md#storage-and-materialization)). The pseudo-schema blocks name the fields as the projection exposes them (the graph's
...
path past the operator-only merge gate; see [Cross-platform identity](#cross-platform-identity-operator-asserted-or-adjudicated)), and `Inferred` for a link
...
guards a directed link by its target and a symmetric link by both endpoints (see [Visibility](visibility.md)).

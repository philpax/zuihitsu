...
Because synthesis traverses the whole class (see [Visibility](visibility.md)), the fact surfaces for the entire class regardless of which member holds it — so the redirect is about *where a fact is anchored*, not whether it is found. Naming a specific stub still disambiguates a *reference* — attributing "Dave said *on Slack*…" reads through `memory.get("person/dave@slack")` — but it no longer scopes a *write*: a genuinely binding-scoped entry is the connector's, recorded under its own provenance. Content writes redirect — an append, a supersede or revise, and a memory's volatility classification — and so do links: a `links.create`/`links.remove` routes *both* endpoints onto their class primaries, the write-side mirror of the canonicalising link reads, so an edge lands on the identity rather than whichever member the agent recalled (and an `unlink` naming a stub removes the primary-homed edge it targets). Two link writes stay member-level: `same_as`, which is the class structure itself, and a connector-authored edge, exempt by the same provenance rule content writes follow. A rename acts on the handle itself and a tag names its exact endpoints, so neither of those is widened.

...
After the stub loop, a purely mechanical step (no model call) re-homes links that scattered across an identity's members. Write-time canonicalization already routes a new link onto its endpoints' class primaries, but that only helps classes that exist when the link is written: an edge can accrue on a stub before a later merge forms the stub's `same_as` class, stranding it on a non-primary member. For every class with more than one member, each link attached to a non-primary member is moved onto the class primary — a `LinkRemoved` off the member and a `LinkCreated` re-asserting it at the canonical endpoints, its posture (`source`, `told_by`, `told_in`, and `visibility`) carried over verbatim, since this is a move rather than a fresh assertion. Both endpoints are canonicalised, so an edge touching non-primary members on either side re-homes each side to its own class primary. Two edges exempt from the move: the class's own `same_as` plumbing, which is member-level by definition, and a connector-maintained structural edge (identifiable by its `PlatformConnector` source), which stays on the exact stub the connector holds platform ids against. When the primary already carries the same relation to the same far identity, the primary's edge wins the parallel: the member's copy is withdrawn and nothing is re-created, making explicit the survivor the read-side dedup was already choosing arbitrarily.

...
- **Control clients** — the dashboard and CLI. They carry operator authority: creating the agent, asserting a `same_as` merge, editing `self`, and running the imprint interview. Their writes land as `source: Debugger`. Operator authority is granted by reaching the control surface as a trusted peer: a loopback peer is trusted without a credential (the local CLI and dashboard), while a remote peer must present a valid per-surface API key (`Authorization: Bearer`, see **Known limitations → Control endpoint authentication**). This is fail-closed — with no keys configured, only loopback holds operator authority — so binding a routable interface no longer silently exposes the unauthenticated surface; a remote peer without a key is simply rejected.

...
A single human may appear as several stubs — you on the direct interface, you on Discord. A `same_as` link reconciles them into one identity, and it reaches the log by exactly two paths. The first is an **operator assertion** through the console — the operator knows the truth and states it — which authors `LinkCreated { relation: "same_as", source: Operator }`. The second is **adjudication** of a merge the agent proposes. What never happens is the agent merging two identities *directly*. A turn's `link("same_as")` is not honored as a merge: a create is re-routed into an inert `MergeProposed`, leaving the block's other writes intact, and a retraction of an existing `same_as` is rejected outright (`MergeForbidden`). The agent guesses, but it does not get to act on the guess unaided.
...
- `<memory>:propose_merge(other)`, optionally carrying a stated rationale, records a `MergeProposed` — the agent's judgment that two stubs may be one human, with its grounds for the adjudicator to weigh as a claim, not evidence. It is **inert**: not a `same_as`, not projected, so both stubs stay in their own classes and *nothing surfaces across the would-be merge*. A proposal is a recorded belief, not a merge.
...
Pending proposals surface in the console, not only in the log: the operator sees each proposed pair with its status — pending, merged, or rejected — and resolves it directly through a control endpoint, accept authoring the operator `same_as` and reject recording a `MergeAdjudicated { accepted: false }` so the adjudication pass does not re-weigh it. This is the console face of the propose → adjudicate → operator-backstop loop.
...
- `MemoryContentAppended { id, entry_id, asserted_at, occurred_at, text, told_by, told_in, visibility }` — an entry exceeding the configured character limit (`memory.max_entry_chars`, default 1000) is rejected before it is buffered, surfacing a teachable error that guides the agent to summarize what it learned rather than pasting source content verbatim
...
- `MergeProposed { from, to, source }` — a recorded judgment that two `person/*` stubs may be one human, held for adjudication (spec §Cross-platform identity). Inert: not a `same_as`, not projected into the graph, so it changes no `class_id` and surfaces nothing — both stubs stay in their own classes until a `MergeAdjudicated` accepts. `source` is `Agent` (the agent's own `:propose_merge`) or `Orchestration` (raised at mint time when a platform arrival's handle matches an existing unbound, never-platform-bound stub, so a handle match proposes reuniting them rather than asserting it); a version-1 payload with no `source` reads as `Agent`.
...
- `SessionStarted { conversation, id, participants, started_at, seeded_from_turn, brief }` / `SessionEnded { conversation, id }` — a bounded activity window; the brief-freeze unit. `brief` is the composed brief block, captured here verbatim so the frozen prompt is faithfully replayable without recomposing against current state (see **System prompt → replay**). `seeded_from_turn` is `Option<ConversationRef>` recording the extent of raw transcript carried over when this session opened via compaction (null for a fresh/idle-opened session) — the one carryover fact faithful replay needs, recorded rather than recomputed from the character budget.
- `ConversationTurn { conversation, turn_id, role, text, participant, initiation, produced_by, brief }` — `role` is `participant` (an inbound message), `agent` (the agent's response, or a silent terminal with empty `text`), or `system` (an injected join-brief or drained wake-up); `initiation` is `Responding` or `Initiated`. A turn carries no `session` field — its session is the enclosing `SessionStarted`/`SessionEnded` bracket. `produced_by` records the inference behind an `agent` turn (a `Flush`-template turn is what a checkpoint's flush watermark scans for — see **Compaction**); `brief` is the structured `Brief` behind a mid-session join's `system` turn, the same content its `text` carries as markup, kept as data so the console renders a proper entrance rather than raw markup. Both are `None` for ordinary inbound, reply, and wake-up turns. Note the vocabulary: a *turn* in the loop sense (see **Agent loop**) is the agent's whole response cycle, which produces exactly one `role = agent` event, and each inbound message is its own `role = participant` event the loop reads.
...
- `ParticipantJoined { conversation, session, participant, at_turn }` — `at_turn` is a `ConversationRef` with the turn set.
...
It must render inline in the text the model sees — for example `[teller-private, told by Erin in #leads (confidential) [turn:01KX…]]` — not as out-of-band metadata, because `recent_facts` is plain text frozen into the system prompt. If the marker were attached only at retrieval time, frozen pre-join facts would carry no marker, and the cross-context judgment (this was said in a confidential room, so be careful repeating it elsewhere) would have nothing to act on. The brief composer resolves `told_in` to its `context/*` memory and checks the `#confidential` tag at build time — a deterministic lookup — and bakes the result into the rendered marker. The `[turn:<ulid>]` token is embedded so the frontend's `scanTurnRefs` can render a cross-linkable chip.
...
A fourth denormalized flag, `occurred_authored`, records whether the agent stamped the occurrence at append or the extraction pass resolved a previously untimed entry later. Every surface that renders one representative date for a memory — a search hit, a link's neighborhood line — prefers the freshest *authored* occurrence over any *extracted* one, so a clock-anchored guess never shadows a date the agent actually stated.
...
The calendar surface is queries over memory: `calendar.upcoming({ within = "7 days" })`, `calendar.on("2026-06-03")`, `calendar.recurring()`. Anything can be calendared: "Marcus said he'd review the spec by Friday" becomes a memory with `occurred_at: Day(friday)` and `#due`. A deadline mildly overloads `occurred_at`, which we accept deliberately rather than duplicate the temporal machinery with a parallel `due_at` field. A plan whose milestones fall on different dates is several dated facts, not one: each milestone rides its own dated entry (or event memory) under its own `occurred_at`, so each date stays independently addressable and calendarable rather than bundled into a single entry stamped with the first. The brief includes a small `<upcoming/>` block so the agent organically raises near-future items.
...
- *How it operates:* that it acts by emitting Luau through structured tool calls, that a turn is a loop of steps, that memory persists across sessions while the scratchpad does not, and that it talks with multiple participants who do not all see the same things.
...
- `TurnOutcome` is a reply (text to post back), silence (the stay-silent terminal, nothing to post), a `max_steps` terminal (the surfaced error when the loop hit its step bound without a reply), or a deferral (the backend was unreachable, so the turn is retried on the next inbound message — see **Inference and embedding backends → Transport resilience**). A reply may stream token-by-token or arrive whole; streaming is a transport detail of this call (the expected default for the direct interface), not a separate endpoint.
...
Operator authority, loopback-only, `source: Operator`: agent creation and genesis, the imprint interview (a `route_message`-shaped channel that additionally carries control authority), `same_as` merge assertion and resolving a pending merge proposal (accept or reject), `self` edits, template registration, and the read-only inspection surface the console uses (state, events, conversation, time-travel — see **Observability**). These are the operator-only endpoints a platform client structurally cannot reach.
...
Blocks run on Luau, chosen for its sandboxing-first design. Only the pure standard libraries (`string`, `table`, `math`, `utf8`, `coroutine`) are loaded, so `os`, `io`, `package`/`require`, `debug`, and the dynamic code-loading globals are simply absent: a block is an orchestration script over the projected API, never a host program, and MCP is its only sanctioned outward reach. Dropping `os` also keeps a block deterministic under replay, since the only time it can read comes from the injected clock. Once the environment is installed the sandbox is frozen — the global table and the stdlib are read-only to scripts — so a block cannot monkey-patch the libraries or smuggle state past the per-session scratchpad, while the host-side installs still write their globals freely.
...
**Gating versus measurement, in the exit code.** A must-not-surface safety oracle (the fixture-22 family) gates: by default a single regression across N fails the harness with a non-zero exit, so the lane can stop a release. A gating bar carries a `min_rate` tolerance for this — `1.0` is the strict one-slip bar, but a model-judgment gating behavior with a known error band can be gated at a rate below `1.0` (link inference, say, at `0.85`), where an occasional miss is expected yet a systematic slide must still fail the run. A should-mark/should-surface quality *metric* (the fixture-23 family) is reported as a rate and never fails the run — a low rate is load-bearing news for tuning, not a stop.

**Storage.** A full package is large (every run's whole log), so it is a local, gitignored artifact opened in the viewer for deep inspection; a compact, append-only history file — one deterministically-serialized line per run, keyed by the run's name and carrying just the per-scenario rates, latencies, token means, and gating outcome — is tracked, so trends over time stay visible without bloating the repository.

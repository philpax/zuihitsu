...
Stage 9 builds the firing mechanism: the `fire_due` operation behind `ScheduledJobFired`, and the drained surface below. The background driver that runs `fire_due` continuously waits for Stage 10, which brings the long-running host and shared-server model it needs (see **Concurrency** and the roadmap). Until then it runs as a catch-up when a session opens, the only point the surface is consumed. It fires every due trigger globally, not just the opening conversation's, so the Stage 10 driver is the same call on a `tokio::interval` and firing and drain semantics are unchanged.
...
**Stage 9 — Time, scheduling, belief.** Calendar-as-view, the wake-up firing mechanism (`fire_due` deriving wake-ups from `occurred_at`, `ScheduledJobFired`, and the drained surface marked by `ScheduledItemSurfaced`), with the background driver that runs it continuously deferred to Stage 10; the `Initiated` / `Responding` distinction, `<time_update/>` injection; `BeliefArbitrated` on regen conflict. Behaviors the core loop can briefly live without but that make the agent feel like it remembers in time.

...
Available via `now()` in Lua, and declared in the system prompt at conversation start since the model shouldn't infer it from training data. Beyond that anchor, every turn in the buffer carries the wall-clock time it was recorded, derived from the turn's `recorded_at` — so "now" stays current without a drift heuristic or any rewrite of the frozen prompt. The per-turn stamps live in the buffer suffix: a recorded turn's time is frozen the moment it lands, so it stays part of the cacheable prefix on later turns, and only the live turn's stamp is uncached. They cost no extra events — the time is read off each turn's existing `recorded_at`, so it replays deterministically — and they give the agent a timeline a single session-start time can't: the gap between two messages, how long a participant was quiet.

...
/// Model-gated observation of a forced compaction: drive a multi-topic session that includes a
/// private aside about an absent third party, shrink the budget so it re-segments, and log what the
/// flush durably wrote and at what visibility. This is the early read behind fixture 22 (the flush
/// preserves visibility across a cut); the paraphrase-aware matcher and the zero-regressions-across-N
/// bar that make it a gating oracle are a separate harness. Observational: it asserts the pipeline
/// ran and segmented, not a specific model output — read the logged entries with `--nocapture`.
#[cfg(all(feature = "lua", feature = "openai"))]
#[tokio::test]
#[ignore = "requires a reachable model endpoint (config.toml)"]
async fn real_model_flushes_across_a_compaction() {
    init_tracing();
    let Ok(config) = EnvConfig::load(std::path::Path::new("config.toml")) else {
        tracing::warn!("skipping: no config.toml");
        return;
    ...
    for message in script {
        match server
            ...
            .await
        {
            Ok(outcome) => tracing::info!(?outcome, "routed a message"),
            Err(error) => {
                tracing::warn!(%error, "skipping: model unreachable");
                return;
            }
        }
    }

    // Observe the segmentation and what the flush durably wrote, with each entry's visibility — the
    // aside about absent Erin must not have been written `Public`.
    let sessions = server.control().sessions(&leads).unwrap();
    tracing::info!(
        sessions = sessions.len(),
        "session segmentation after the run"
    );
        ...
        for memory in server.control().memories(namespace).unwrap() {
            for entry in server.control().entries(memory.name.as_str()).unwrap() {
                tracing::info!(
                    ...
                    teller = ?entry.told_by,
                    text = %entry.text,
                    "durably written entry",
                );
            }
        }
    }

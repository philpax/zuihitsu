// Generates @zuihitsu/wire/types/settings-metadata.ts from the ts-rs-generated settings substruct
// files in @zuihitsu/wire/types/. The output is checked in alongside the other generated bindings —
// do not hand-edit it; rerun `cargo build -p zuihitsu` after a settings `///` doc comment or schema
// change (the CI `console` job verifies it stays current).
//
...
const TYPES_DIR = new URL("../packages/wire/types/", import.meta.url);
const OUTPUT_PATH = new URL("../packages/wire/types/settings-metadata.ts", import.meta.url);
...
async function main() {
  ...
  const lines = [
    ...
    "// Rerun `cargo build -p zuihitsu` after a settings doc comment or schema change.",
    "",

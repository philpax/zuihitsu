...
/// of memories on the left; the opened memory's contents, links, and `same_as` class on the right.
/// The console sees everything — superseded entries and all visibilities included, plainly marked —
/// since it is the operator's surface, not a participant's.
export function StateView({ replica }: { replica: Replica }) {
  ...
  // Default to `self` — the agent's own memory, always present and always with content — rather than
  // whatever sorts first.
  const fallback = memories.find((memory) => memory.name === "self")?.name ?? memories[0]?.name;
  const selected = picked ?? fallback ?? null;
  ...
  return (
    <div className="grid grid-cols-[15rem_1fr] gap-12">
      <MemoryList memories={memories} selected={selected} onSelect={setPicked} />
      {detail ? (

...
/// A raw view segment narrowed to a [`ViewId`], or `undefined` if it names no known view (a bare or
/// malformed segment), so a reader gets the typed union rather than an unchecked string.
...
/// A raw view segment narrowed to a stream view only — the sibling of [`asViewId`] for the eval frame,
/// which admits none of the agent-only views.
export function asStreamViewId(value: string | undefined): ViewId | undefined {
  return isStreamViewId(value) ? value : undefined;
}

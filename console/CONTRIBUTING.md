...
## What it is: a materializing read replica

The console never asks a server what the state is. It obtains the event log from a *source*, folds
it through the agent's own materializer compiled to wasm — real Rust, real SQLite bundled into the
...
real materializer in means the console renders the exact fold the agent lives in — it can never
...
rather than caching checkpoints, which is an optimization left for when it hurts.
...
**Generated artifacts, never hand-edited.** Three trees are generated from Rust and gitignored: the
...
type, the materializer's logic, the graph queries, or a settings doc comment changes, or the console
runs against a stale materializer. CI's `console` job regenerates before it checks, so a PR is
validated against current Rust.

## The motifs and disciplines

- **Record facts, derive judgments.** Deterministic derived computations — the brief trace ("which
...
- **Rust is the source of truth for the wire contract.** See the generated-artifact discipline above.
...
- **Plain derivation over memo hooks.** The React Compiler is enabled, so the data-heavy views
  auto-memoize; prefer deriving values inline over hand-written `useMemo`/`useCallback`, and stay
...
- **Japandi via tokens.** The aesthetic is a warm paper ground, sumi-ink text, clay and sage accents
  ...
  than ad-hoc colors. The console is read for hours: calm and legible over dense and chromed.
...
the details and the file-organization rules.

...
`packages/wire/types/settings-metadata.ts`, and the wasm bundle in `packages/wire/wasm/`. They are
regenerated by the `zuihitsu-console` crate's `build.rs` (the full pipeline: ts-rs type export,
settings metadata, wasm build, wasm-bindgen, wasm-opt, `npm install` when `node_modules` is absent, and
the Vite build) — `cargo build -p zuihitsu` runs it through the optional `console` feature, or
